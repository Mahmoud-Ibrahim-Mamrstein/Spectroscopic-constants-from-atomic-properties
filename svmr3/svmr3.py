...
def ml_model(data,strata,test_size,constant_value,length_scale,C,epsilon,nu,features,logtarget,target,n_splits=1000):
    ...
    for train_index, test_index in mcs.split(data,strata):
        ...
        for i in re_train_set['Molecule'].isin([re_test_set['Molecule']]):
            if i ==True:
                print('Warning: One molecule is in the training set and testing set at the same time')
...
g,gr,gw, g_old, g_new, gr_old, gw_old, gr_new, gw_new, g_expand, gr_expand, gw_expand, g_old_expand, g_new_expand, gr_old_expand, gw_old_expand, gr_new_expand, gw_new_expand=load(handel=r"svmr3_gw_expand_pred.csv",old_handel=r"list of molecules used in Xiangue and Jesus paper.csv")
...
params['kernel__k1__k2__length_scale'] = (0.1, 1000.0, 'log-uniform')
params['kernel__k1__k1__constant_value'] = (0.1, 1000.0, 'log-uniform')
...
pyplot.savefig('svmr3_annot.svg')


# In[ ]:


results('svmr3 model',gr_expand,'Re (\AA)',re_test_preds,308,MAE,RMSE,R,r"stat_summ.csv")
gr_expand['re_test_preds']=re_test_preds
gr_expand['re_train_preds']=re_train_preds
gr_expand.to_csv('svmr3_gr_expand_pred.csv')
split_stat = pd.DataFrame(list(zip(Train_MAE,Train_RMSE,MAE,RMSE)),columns =['Train_MAE','Train_RMSE','MAE','RMSE'])
split_stat.to_csv('svmr3_split_stat.csv')
