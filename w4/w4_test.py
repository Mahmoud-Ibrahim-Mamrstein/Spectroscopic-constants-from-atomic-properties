...
g,gr,gw, g_old, g_new, gr_old, gw_old, gr_new, gw_new, g_expand, gr_expand, gw_expand, g_old_expand, g_new_expand, gr_old_expand, gw_old_expand, gr_new_expand, gw_new_expand=load(handel=r"w4_gw_expand_test.csv",old_handel=r"list of molecules used in Xiangue and Jesus paper.csv")
...
gw_expand['ln(w)']=np.log(gw_expand['omega_e (cm^{-1})']) #The natural logaritm of the square root of $\omega_e$ will be used as a target
...
trval,train,test,mean_std,Train_MAE,Train_RMSE,Train_R,Train_RMSLE,MAE,RMSE,R,RMSLE,r_y_train_preds,r_train_stds,r_y_test_preds,r_test_stds=ml_model(data=gw_expand,features=['Re (au)','p1','p2','g1_lan_act','g2_lan_act','mu^(1/2)'],prior_features=['Re (au)','ln(mu^(1/2))','p1','p2','g1_lan_act','g2_lan_act'],target='omega_e (cm^{-1})',logtarget='ln(w)',nu=3/2,normalize_y=True)
