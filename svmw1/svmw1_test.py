...
g,gr,gw, g_old, g_new, gr_old, gw_old, gr_new, gw_new, g_expand, gr_expand, gw_expand, g_old_expand, g_new_expand, gr_old_expand, gw_old_expand, gr_new_expand, gw_new_expand=load(handel=r"svmw1_gw_expand_test.csv",old_handel=r"list of molecules used in Xiangue and Jesus paper.csv")
...
best_params_=([('C', 100), ('epsilon', 0.0001), ('kernel__k1__k1__constant_value', 62), ('kernel__k1__k2__length_scale', 22)]) # the best parameters rturned from the Bayes search cv,these results may be used to replicate the results in the manuscript
trval,train,test,Train_MAE,Train_RMSE,Train_R,Train_RMSLE,MAE,RMSE,R,RMSLE,r_y_train_preds,r_y_test_preds=ml_model(data=gw_expand,C=best_params_[0][1],epsilon=best_params_[1][1],constant_value=best_params_[2][1],length_scale=best_params_[3][1],nu=3/2,features=['p1','p2','g1_lan_act','g2_lan_act','mu^(1/2)'],target='omega_e (cm^{-1})',logtarget='ln(w)')
...
true_values=gw_expand[gw_expand['Molecule'].isin(['MoC','NbC','NiC','NiO','NiS','PbI','PdC','RuC','SnI','UO','WC','YC','ZnBr','ZnCl','WO','ZnI','ZnF','HCl','DCl'])]['omega_e (cm^{-1})'].tolist()
re_test_preds=[]
re_test_std=[]
out=[]
i=(-1)
...
testing_results = pd.DataFrame(list(zip(test_molecules, true_values,re_test_preds)), columns =['Molecule', 'true $\omega_e$(cm$^{-1}$)','Predicted $\omega_e$(cm$^{-1}$)']).drop_duplicates(subset='Molecule')
testing_results.to_csv(r'svmw1_testing_results.csv')
