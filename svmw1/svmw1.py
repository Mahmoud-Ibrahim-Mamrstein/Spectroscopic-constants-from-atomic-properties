...
g,gr,gw, g_old, g_new, gr_old, gw_old, gr_new, gw_new, g_expand, gr_expand, gw_expand, g_old_expand, g_new_expand, gr_old_expand, gw_old_expand, gr_new_expand, gw_new_expand=load(handel=r"svmw1_gw_expand_pred.csv",old_handel=r"list of molecules used in Xiangue and Jesus paper.csv")
...
search.fit(gw_expand[['p1','p2','g1_lan_act','g2_lan_act','mu^(1/2)']], gw_expand['ln(w)'])
...
# 0.9937916854074417
# OrderedDict([('C', 100), ('epsilon', 0.0001), ('kernel__k1__k1__constant_value', 62), ('kernel__k1__k2__length_scale', 22)])
# 
...
trval,train,test,Train_MAE,Train_RMSE,Train_R,Train_RMSLE,MAE,RMSE,R,RMSLE,r_y_train_preds,r_y_test_preds=ml_model(data=gw_expand,strata=gw_expand['wcat'],test_size=30,C=best_params_[0][1],epsilon=best_params_[1][1],constant_value=best_params_[2][1],length_scale=best_params_[3][1],nu=3/2,features=['p1','p2','g1_lan_act','g2_lan_act','mu^(1/2)'],target='omega_e (cm^{-1})',logtarget='ln(w)',n_splits=1000)
...
pyplot.savefig('svmw1_annot.svg')


# In[ ]:


results('svmw1 model',gr_expand,'Re (\AA)',re_test_preds,314,MAE,RMSE,R,r"stat_summ.csv")
gr_expand['we_test_preds']=re_test_preds
gr_expand['we_train_preds']=re_train_preds
gr_expand.to_csv('svmw1_gw_expand_pred.csv')
split_stat = pd.DataFrame(list(zip(Train_MAE,Train_RMSE,MAE,RMSE)),columns =['Train_MAE','Train_RMSE','MAE','RMSE'])
split_stat.to_csv('svmw1_split_stat.csv')
