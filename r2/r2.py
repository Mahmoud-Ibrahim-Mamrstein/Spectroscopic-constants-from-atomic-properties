...
g,gr,gw, g_old, g_new, gr_old, gw_old, gr_new, gw_new, g_expand, gr_expand, gw_expand, g_old_expand, g_new_expand, gr_old_expand, gw_old_expand, gr_new_expand, gw_new_expand=load(handel=r"r2_gr_expand_pred.csv",old_handel=r"list of molecules used in Xiangue and Jesus paper.csv")
...
gr_expand=gr_expand[~gr_expand['Molecule'].isin(['XeCl','AgBi','Hg2','HgCl'])] #Remmove molecules with uncertain data
gr_expand['rcat']=gr_expand['Re (\AA)'] # gr_expand['rcat'] is used to define strata for the process of stratified sampling
gr_expand_unique=np.unique(gr_expand['rcat'])
...
gr_expand['mu^(1/2)']=(np.sqrt(gr_expand['Reduced mass'])) #square root of the reduced mass will be used as a feature
...
re_train_preds,re_train_std,re_test_preds,re_test_std,out,fig,ax=plot_results(gr_expand,'True $R_e(\AA)$','Predicted $R_e(\AA)$','Re (\AA)',r_y_train_preds,r_train_stds,r_y_test_preds,r_test_stds);
pyplot.savefig('r2_scatter.svg')
for i in range(len(re_test_preds)):
    if abs(gr_expand['Re (\AA)'].tolist()[i]-re_test_preds[i])<0.1:
        continue
    ax.annotate(gr_expand['Molecule'].tolist()[i], (gr_expand['Re (\AA)'].tolist()[i], re_test_preds[i]))
pyplot.savefig('r2_scatter_ann.svg')
...
gr_expand['re_train_std']=re_train_std
gr_expand.to_csv('r2_gr_expand_pred.csv')
split_stat = pd.DataFrame(list(zip(Train_MAE,Train_RMSE,MAE,RMSE)),columns =['Train_MAE','Train_RMSE','MAE','RMSE'])
split_stat.to_csv('r2_split_stat.csv')
