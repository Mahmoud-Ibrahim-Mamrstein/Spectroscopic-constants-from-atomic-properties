...
def ml_model(data,constant_value,length_scale,C,epsilon,nu,features,logtarget,target): #function used for implementing the leave one out GPR model for testing
    ...
    for train_index, test_index in loo.split(data): # leave one out is used for testing. That is the whole data set is used in the kernel to make one prediction
        ...
        if re_test_set['Molecule'].tolist()[0] not in ['MoC','NbC','RuC','SnI','YC','ZnBr']:
            continue # only make prediction is the molecule is in the following list ['MoC','NbC','RuC','SnI','YC','ZnBr']
...
g,gr,gw, g_old, g_new, gr_old, gw_old, gr_new, gw_new, g_expand, gr_expand, gw_expand, g_old_expand, g_new_expand, gr_old_expand, gw_old_expand, gr_new_expand, gw_new_expand=load(handel=r"svmd1_gw_expand_test.csv",old_handel=r"list of molecules used in Xiangue and Jesus paper.csv")
...
trval,train,test,Train_MAE,Train_RMSE,Train_R,Train_RMSLE,MAE,RMSE,R,RMSLE,r_y_train_preds,r_y_test_preds=ml_model(data=gw_expand,C=best_params_[0][1],epsilon=best_params_[1][1],constant_value=best_params_[2][1],length_scale=best_params_[3][1],nu=3/2,features=['p1','p2','g1_lan_act','g2_lan_act'],target="D0 (eV)",logtarget="ln(D0 (eV))")


# In[ ]:


test_molecules=gw_expand[gw_expand['Molecule'].isin(['MoC','NbC','RuC','SnI','YC','ZnBr'])]['Molecule'].tolist()
true_values=gw_expand[gw_expand['Molecule'].isin(['MoC','NbC','RuC','SnI','YC','ZnBr'])]["D0 (eV)"].tolist()
re_test_preds=[]
out=[]
i=(-1)
s=0
...
testing_results = pd.DataFrame(list(zip(test_molecules, true_values,re_test_preds)), columns =['Molecule', 'true $D_{o}^{o}$ (eV)','Predicted $D_{o}^{o}$ (eV)'])
testing_results.to_csv(r'svmd1_testing_results.csv')
