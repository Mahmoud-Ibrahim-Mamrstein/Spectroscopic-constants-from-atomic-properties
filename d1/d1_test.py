...
def ml_model(data,features,prior_features,logtarget,target,nu,normalize_y): #function used for implementing the leave one out GPR model for testing
    ...
    for train_index, test_index in loo.split(data): # leave one out is used for testing. That is the whole data set is used in the kernel to make one prediction
        ...
        if re_test_set['Molecule'].tolist()[0] not in ['MoC','NbC','RuC','SnI','YC','ZnBr']:
            continue # only make prediction is the molecule is in the following list ['InBr','MoC','NbC','NiC','NiO','NiS','PbI','PdC','RuC','SnI','UO','WC','YC','ZnBr','ZnCl','WO','ZnI','ZnF','HCl','DCl']
...
g,gr,gw, g_old, g_new, gr_old, gw_old, gr_new, gw_new, g_expand, gr_expand, gw_expand, g_old_expand, g_new_expand, gr_old_expand, gw_old_expand, gr_new_expand, gw_new_expand=load(handel=r"d1_gw_expand_test.csv",old_handel=r"list of molecules used in Xiangue and Jesus paper.csv")
...
testing_results = pd.DataFrame(list(zip(test_molecules, true_values,re_test_preds,re_test_std)), columns =['Molecule', 'true $D_{o}^{o}$)','Predicted "$D_{o}^{o}$ (eV)")','error bars']).drop_duplicates(subset='Molecule')
testing_results.to_csv(r'd1_testing_results.csv')
