...
# ## 3.2 Function that perfroms the MC splits and train fit the SVR and make predictions

# In[ ]:


def ml_model(data,strata,test_size,constant_value,length_scale,C,epsilon,features,logtarget,target,n_splits=1000):
    r_y_train_preds={}
    r_y_test_preds={}
    ...
    for train_index, test_index in mcs.split(data,strata):
        ...
        kernel = kernel=ConstantKernel(constant_value=constant_value)*Matern(length_scale=length_scale, nu=3/2)+WhiteKernel(noise_level=re_train_set[target].std()/np.sqrt(2),noise_level_bounds=(10**-15,1))
...
g,gr,gw, g_old, g_new, gr_old, gw_old, gr_new, gw_new, g_expand, gr_expand, gw_expand, g_old_expand, g_new_expand, gr_old_expand, gw_old_expand, gr_new_expand, gw_new_expand=load(handel=r"r1_svr_gr_expand_pred.csv",old_handel=r"list of molecules used in Xiangue and Jesus paper.csv")
...
pyplot.savefig('r1_annot.svg')


# In[ ]:


results('r1_svr model',gr_expand,'Re (\AA)',re_test_preds,314,MAE,RMSE,R,r"stat_summ.csv")
gr_expand['re_test_preds']=re_test_preds
gr_expand['re_train_preds']=re_train_preds
gr_expand.to_csv('r1_svr_gr_expand_pred.csv')
split_stat = pd.DataFrame(list(zip(Train_MAE,Train_RMSE,MAE,RMSE)),columns =['Train_MAE','Train_RMSE','MAE','RMSE'])
split_stat.to_csv('r1_svr_split_stat.csv')
